...
gameEngine.addResource({ id: 'batteries', name: 'Batteries', icon: '🔋', startValue: 0, displayPrecision: 0, coreKey: 'batteries' });
gameEngine.addResource({ id: 'batteriesPerSecond', name: 'Production', icon: '📈', startValue: 0, displayPrecision: 1, suffix: '/s', coreKey: 'batteriesPerSecond' });
gameEngine.addResource({ id: 'techDebt', name: 'Tech Debt', icon: '⚠️', startValue: 0, displayPrecision: 0, coreKey: 'techDebt' });
gameEngine.addResource({ id: 'coffee', name: 'Coffee Today', icon: '☕', startValue: 0, displayPrecision: 0, coreKey: 'coffee' });

// === Core Tab Definitions ===
gameEngine.addTab({ id: 'production', name: 'Production', icon: '🔋', unlockCondition: () => true, core: true });
gameEngine.addTab({ id: 'monitoring', name: 'Monitoring', icon: '📊', unlockCondition: (gs) => gs.grafanaUnlocked, core: true });
gameEngine.addTab({ id: 'plugins', name: 'Plugins', icon: '🔌', unlockCondition: () => true, core: true });
...
async function loadPluginFiles() {
  try {
    ...
    console.log(`[Plugins] Manifest loaded: ${data.plugins?.length || 0} plugin(s) listed`);
    if (data.plugins && data.plugins.length > 0) {
      for (const file of data.plugins) {
        const sizeBefore = PluginRegistry.plugins.size;
        await new Promise((resolve) => {
          const script = document.createElement('script');
          script.src = `plugins/${file}?v=${Date.now()}`;
          script.onload = () => {
            if (PluginRegistry.plugins.size > sizeBefore) {
              console.log(`[Plugins] ${file} loaded and registered successfully`);
            } else {
              console.warn(`[Plugins] ${file} loaded but did not register - check for JS errors in console`);
            }
            resolve();
          };
          script.onerror = () => { console.error(`[Plugins] Failed to load script: plugins/${file}`); resolve(); };
          document.head.appendChild(script);
        });
      }
    }
  } catch (e) {
    console.warn('[Plugins] Could not load manifest.json -', e.message, '(are you using a local server? file:// won\'t work)');
  }
  console.log(`[Plugins] ${PluginRegistry.plugins.size} plugin(s) registered, attempting to load...`);
...
const BESSTycoon = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setGameState(prev => {
        ...
        // Check for newly-unlocked plugins
        const newPlugin = PluginRegistry.checkAndLoadNewPlugins(gameEngine, state);
        if (newPlugin) {
          state.events = [{ text: `🔌 Plugin unlocked: ${newPlugin}!`, time: Date.now() }, ...state.events.slice(0, 9)];
        }
  ...
  const buyUpgrade = (upgradeId, cost, debtCost = 0) => {
    setGameState(prev => {
      ...
      if (upgradeId === 'intern') {
        ...
        eventText = "Bulk order from Alibaba! Datasheets are... creative. 📦";
      } else if (upgradeId === 'ignoreCerts') {
        state.upgrades.ignoreCerts = true;
        eventText = "Certifications are just suggestions, right? 🤷";
        ...
        eventText = "🎉 Grafana unlocked! Time to make dashboards nobody understands!";
      ...
      if (achievement && !state.achievements.find(a => a.id === achievement.id)) {
        ...
        state.events = [{ text: `🏆 Achievement: ${achievement.name}`, time: Date.now() }, ...state.events.slice(0, 9)];
      }
  ...
  const buyPluginUpgrade = (upgradeDef) => {
    setGameState(prev => {
      ...
      if (upgradeDef.oneTime) {
        ...
        const countKey = `upgrade_count_${upgradeDef.id}`;
        state.pluginData = { ...state.pluginData, [countKey]: (state.pluginData[countKey] || 0) + 1 };
      }
      return state;
    });
  };
  ...
  const getRandomEvent = (state) => {
    const pool = [
      "☕ Drank too much coffee. Hands shaking. Batteries shaking. Everything shaking.",
      "📧 CEO sent 3 AM Slack: 'Quick question...' (47 paragraphs follow)",
      "🔥 Intern configured Modbus by guessing. 12 racks now identify as toasters.",
      "📊 Investor asked for metrics. You showed them a Grafana dashboard with no data.",
      "🐛 Found bug in production. Fix it? Or call it a feature?",
      "🔌 Customer called: 'Why is my battery at 140% charge?' Good question.",
      "💾 Forgot to commit. Lost 3 hours of work. Git is not your friend today.",
    ];
    if (state.upgrades.skipTesting && Math.random() < 0.3) {
      pool.push("⚠️ Skipping testing caused a minor incident. MINOR. Totally minor.");
    }
    if (state.upgrades.ignoreCerts && Math.random() < 0.2) {
      pool.push("🚨 Regulator noticed your lack of certifications. They're 'monitoring the situation.'");
    }
    if (state.grafanaUnlocked && Math.random() < 0.25) {
      const grafanaEvents = [
        "📉 Grafana query took 45 seconds. Coffee break!",
        "🔴 Alert: Temperature spike! (It was just someone's laptop near the sensor)",
        "📊 Created 47 new dashboard panels. Still can't find the useful one.",
        "⚠️ InfluxDB retention policy expired. 6 months of data? Gone!",
      ];
  ...
  for (const [category, upgrades] of gameEngine.upgrades) {
    for (const upg of upgrades) {
      const isOwned = upg.oneTime ? gameState.pluginData[`upgrade_${upg.id}`] === true : false;
      const ownedCount = upg.oneTime ? (isOwned ? 1 : 0) : (gameState.pluginData[`upgrade_count_${upg.id}`] || 0);
      if (upg.unlockCondition && !upg.unlockCondition(gameState)) continue;
      const costs = upg.cost || {};
      // Check if ALL resource costs are affordable (not just money)
      let canAfford = true;
      const extraCosts = [];
      for (const [resId, amount] of Object.entries(costs)) {
        if (resId === 'money') continue;
        const have = gameState.resources[resId] ?? 0;
        if (have < amount) canAfford = false;
        const resDef = gameEngine.resources.get(resId);
        extraCosts.push({ id: resId, name: resDef?.name || resId, amount, have, affordable: have >= amount });
      }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {showExportImport && (
          <div className="mb-6 bg-slate-800 p-4 rounded-lg border border-blue-500/30">
            <h3 className="text-lg font-bold mb-3 text-blue-400">💾 Backup & Restore</h3>
            ...
            <div className="mt-3 p-3 bg-yellow-900/20 rounded border border-yellow-500/30">
              <p className="text-xs text-gray-400">
                💡 <span className="text-yellow-400">Pro tip:</span> Export your save before making risky decisions. 
            ...
            <div className="space-y-6">
              ...
              {gameState.achievements.length > 0 && (
                <div className="bg-slate-800 p-4 rounded-lg border border-yellow-500/30">
                  <h3 className="text-lg font-bold mb-3 text-yellow-400">🏆 Achievements</h3>
            ...
            <div className="bg-slate-800 p-6 rounded-lg border border-cyan-500/30">
              ...
              <div className="space-y-3">
                {[...upgradesData, ...pluginUpgradesData].map(upgrade => {
                  const moneyOk = gameState.money >= upgrade.cost;
                  const resourcesOk = upgrade.canAffordResources !== false;
                  const canBuy = moneyOk && resourcesOk && (!upgrade.oneTime || upgrade.owned === 0);
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <div className="font-bold text-white">
                            ...
                            {isOwned && <span className="ml-2 text-green-400">✓</span>}
                        ...
                        <div className="text-right">
                          {upgrade.cost > 0 && (
                            <div className={`font-mono text-sm ${moneyOk ? 'text-green-400' : 'text-red-400'}`}>
                              ${upgrade.cost.toLocaleString()}
                            </div>
                          )}
                          {upgrade.extraCosts && upgrade.extraCosts.map(ec => (
                            <div key={ec.id} className={`font-mono text-xs ${ec.affordable ? 'text-blue-400' : 'text-red-400'}`}>
                              {ec.amount} {ec.name} ({Math.floor(ec.have)}/{ec.amount})
                            </div>
                          ))}
              ...
              <div className="mt-4 p-3 bg-slate-900 rounded border border-red-500/30">
                <p className="text-xs text-gray-400">
                  💡 <span className="text-red-400">Tech Debt</span> accumulates with shortcuts. 
        ...
        {activeTab === 'plugins' && (
          <div className="space-y-6">
            <div className="bg-slate-800 p-6 rounded-lg border border-purple-500/30">
              <h2 className="text-2xl font-bold mb-4 text-purple-400">🔌 Plugin Manager</h2>
              ...
              {PluginRegistry.plugins.size === 0 ? (
                <div className="bg-slate-900 p-6 rounded text-center">
                  ...
                  <p className="text-gray-400">No plugins detected.</p>
                  <p className="text-gray-500 text-sm mt-1">Make sure you're running from a local server (npx serve .) — file:// won't load plugins.</p>
                  <p className="text-gray-500 text-sm mt-1">Check the browser console for [Plugins] messages.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  {Array.from(PluginRegistry.plugins.values()).map(plugin => {
                    ...
                    return (
                      <div key={manifest.id} className={`p-4 rounded-lg border ${isLoaded ? 'bg-slate-900 border-green-500/30' : 'bg-slate-900/50 border-slate-700'}`}>
                        ...
                        {manifest.unlockHint && (
                          <div className={`mt-2 text-xs px-2 py-1 rounded ${isLoaded ? 'text-green-500/70 bg-green-900/20' : 'text-yellow-500/80 bg-yellow-900/20'}`}>
                            {isLoaded ? '✅' : '🔒'} Requires: {manifest.unlockHint}
                          </div>
                        )}
                      </div>
                    );
        ...
        <div className="mt-6 text-center text-gray-500 text-sm space-y-1">
          <p>A satirical tribute to Universal Paperclips & the chaos of DevOps 💚</p>
          {lastSave && (
